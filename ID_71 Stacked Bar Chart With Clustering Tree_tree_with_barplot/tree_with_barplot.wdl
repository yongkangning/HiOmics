...
task task_tree_with_barplot {
  input {
    ...
    String y_title
    String plot_title
    String picture1_width
    String picture2_width
  ...
  command <<<
    ...
    # genarate the result
    Rscript ~{appBinDir}/tree_with_barplot_docker_v.1.0.R ~{RLibPath} inputFile2.txt ~{is_prop} ~{dist_method} ~{hclust_method} ~{y_title} ~{plot_title} ~{picture1_width} ~{picture2_width} ~{outputFileName}
...
workflow henbio_wf {
  call task_tree_with_barplot

  output {
    File outFile = task_tree_with_barplot.outFile
  }
}