...
task task_volcano {
  ...
  command <<<
    ...
    # genarate the result
    Rscript ~{appBinDir}/volcano_docker_v.1.1.R ~{RLibPath} inputFile2.txt ~{x_lab} ~{y_lab} ~{p_filter} ~{FC_filter} ~{point_size} ~{point_alpha} ~{x_title} ~{y_title} ~{plot_title} ~{is_legend} ~{is_label} ~{max_overlaps} ~{outputFileName}
...
workflow henbio_wf {
  ...
  output {
    File outFile = task_volcano.outFile
  }
}