...
if (length(args)!=argsCount) {
  stop("metaprop.R libPath inputFile is_sm")
}
.libPaths(args[1])
...
library(meta)
meta1 <- metaprop (data[,2] ,data[,3], data=data, studlab=data[,1],sm=is_sm)
svg(filename = "Forest.svg",height =length(data[,1])*0.7 ,width = 12)
forest(meta1,col.diamond.fixed="lightslategray",
...
metabias (meta1 ,method.bias="linreg",plotit=TRUE, k.min=length(data[,1]))
dev.off()
