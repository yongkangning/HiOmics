...
class myclf_LR:
    ...
    def LR_fit():
        global LR_clf
        LR_clf = LogisticRegression(multi_class='multinomial', solver='sag', penalty='l2', max_iter=10000)
        LR_clf.fit(X_train, y_train)

    def visualization():
        global LR_clf,  X_train, y_train
        LR_clf = LogisticRegression(multi_class='multinomial', solver='sag', penalty='l2', max_iter=10000)
        LR_clf.fit(X_train.iloc[:, :2], y_train)

        
        x_min, x_max = X_train.iloc[:, 1].values.min() - 1, X_train.iloc[:, 1].values.max() + 1
        y_min, y_max = X_train.iloc[:, 0].values.min() - 1, X_train.iloc[:, 0].values.max() + 1

        h = 0.02  

        
        xx, yy = np.meshgrid(np.arange(x_min.item(), x_max.item(), h), np.arange(y_min.item(), y_max.item(), h))

        
        Z = LR_clf.predict(np.c_[yy.ravel(), xx.ravel()])
        ...
        plt.scatter(X_train.values[:, :1], X_train.values[:, 0], c=y_train, edgecolors='k', cmap=plt.cm.Paired)
    ...
    # cv_diy
    def LR_fit_cv(cv_times=5, cv_scoring=None):
...
def train_mode(input_file1):
    ...
    if cv_TF == 'T':
        ...
        if LR_diy_TF == 'T':
            ...
            myclf_LR.LR_fit_diy(LR_penalty
                   , LR_multi_class
                   , LR_C
                   , LR_fi
                   , LR_max_iter
...
if LR_diy_TF == 'T':
    LR_penalty = argv[12]
    LR_multi_class= argv[13]
    LR_C = float(argv[14])
    LR_fi = argv[15]
    LR_tol = float(argv[16])
    LR_max_iter= argv[17]
    LR_solver= argv[18]
