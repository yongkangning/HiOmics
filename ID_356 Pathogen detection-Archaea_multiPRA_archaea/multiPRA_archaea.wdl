...
task task_multiPRA_archaea {
    ...
    command <<<
        ...
        -p ${cpu_cores} \
        --db Archaea -O $HOME/result

        #cp $HOME/result/report.docx  ./
        find $HOME/result/bak/mg/stat/ -name "positive.strain.*.txt" -exec mv {} ./pathogene.archaea.xls \;
    >>>
    ...
    output {
        ...
        File? result = "pathogene.archaea.xls"
    }
