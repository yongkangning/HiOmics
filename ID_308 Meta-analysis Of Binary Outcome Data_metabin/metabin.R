...
if (length(args)!=argsCount) {
  stop("metabin.R libPath inputFile is_sm")
}
.libPaths(args[1])
...
meta3 <-metabin(data[,2],data[,3],data[,4],data[,5],
    data = data,
    studlab = data[,1],
    sm = is_sm,
    method = "I"
...
metabias(meta3,plotit=TRUE,k.min = length(data[,1]))
dev.off()

svg(filename = "Sensitivity.svg",height =length(data[,1])*0.7 ,width = 12)
forest(metainf(meta3),
       col.diamond.fixed="lightslategray",
       col.diamond.lines.fixed="lightslategray",
       col.diamond.random="maroon",
       col.diamond.lines.random="maroon",
