...
task task_multiPRA_bacteria {
    ...
    command <<<
        ...
        -p ${cpu_cores} \
        --db Bacteria -O $HOME/result

        #cp $HOME/result/report.docx  ./
        find $HOME/result/bak/mg/stat/ -name "positive.strain.*.txt" -exec mv {} ./pathogene.bacteria.xls \;
    >>>
    ...
    output {
        ...
        File? result = "pathogene.bacteria.xls"
    }
