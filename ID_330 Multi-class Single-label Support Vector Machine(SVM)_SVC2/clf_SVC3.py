...
class myclf_svc:
    ...
    def scv_fit_cv(cv_times=cv_times,cv_scoring=None):
        global SVC_clf,best_para
        
        parameters = {"estimator__kernel": ["rbf","linear","poly","sigmoid"]}
        svc_clf = SVC(kernel=parameters, probability=True, random_state=0)

        clf = OneVsRestClassifier(svc_clf)
        SVC_clf = GridSearchCV(clf,param_grid=parameters
                                , n_jobs=-1
                                , cv=cv_times
                                , verbose=1
                                , scoring=cv_scoring)
        SVC_clf.fit(X_train, y_train)
        best_para = list(SVC_clf.best_params_.values())
