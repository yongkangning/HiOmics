...
task task_Ternary_Plot {
  ...
  command <<<
    ...
    # genarate the result
    Rscript ~{appBinDir}/Ternary_Plot.R ~{RLibPath} inputFile1.csv ~{is_alpha} ~{group_title} ~{ave_title} ~{outputFileName}
...
workflow henbio_wf {
  call task_Ternary_Plot

  output {
    File outFile = task_Ternary_Plot.outFile
  }
}