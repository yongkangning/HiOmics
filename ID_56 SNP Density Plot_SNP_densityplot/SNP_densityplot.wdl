...
task task_SNP_densityplot {
  ...
  command <<<
    ...
    # genarate the result
    Rscript ~{appBinDir}/SNP_densityplot_docker_v.1.0.R ~{RLibPath} inputFile2.txt ~{is_binsize} ~{outputFileName}
...
workflow henbio_wf {
  call task_SNP_densityplot

  output {
    File outFile = task_SNP_densityplot.outFile
  }
}