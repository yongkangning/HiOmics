...
task task_M_catter_margin_density {
  input {
    String inputFile
    String x_lab
    String y_lab
  ...
  command <<<
    ...
    # genarate the result
    Rscript ~{appBinDir}/scatter_margin_docker2.R ~{RLibPath} inputFile2.txt ~{x_lab} ~{y_lab} ~{is_group} ~{is_alpha} ~{mar_type} ~{mar_alpha} ~{outputFileName}
...
workflow henbio_wf {
  call task_M_catter_margin_density

  output {
    File outFile = task_M_catter_margin_density.outFile
  }
}