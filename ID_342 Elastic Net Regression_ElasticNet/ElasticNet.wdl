...
task task_ElasticNet {
  ...
  command <<<
    ...
    python ~{appBinDir}/main.py -choose ~{choose} -l ~{Y_lb} -fi ~{file} -model ~{model} -ra ~{rate} -cvtime ~{cvtime} -sf ~{shuffle} -Z ~{Zscore} -fit ~{filet} -A ~{alpha} -L ~{l1_ratio}
  >>>
  output {
    File outFile2 = 'Linear.pkl'
    File outFile3 = 'load_model_report.txt'
    File outFile4 = 'ModelPredictData.txt'
    File outFile5 = 'ElasticNet.svg'
    File outFile6 = 'ElasticNet.svg'
