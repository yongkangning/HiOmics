...
task task_bangbangtangtu {
  ...
  command <<<
    ...
    # genarate the result
    Rscript ~{appBinDir}/bangbangtangtu_docker_v.1.0.R ~{RLibPath} inputFile2.txt ~{x_lab} ~{y_lab} ~{have_group} ~{group} ~{have_size} ~{is_size} ~{outputFileName}
...
workflow henbio_wf {
  call task_bangbangtangtu

  output {
    File outFile = task_bangbangtangtu.outFile
  }
}