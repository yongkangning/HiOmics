from sklearn.metrics import classification_report
from sklearn.metrics import confusion_matrix
import pandas as pd
import os,sys
from sklearn.model_selection import GridSearchCV
import zipfile
from sklearn import model_selection
import joblib
from sklearn.utils import shuffle
from sklearn.ensemble import RandomForestClassifier, GradientBoostingClassifier
from sklearn.linear_model import LogisticRegression
from sklearn.tree import DecisionTreeClassifier
from sklearn.neighbors import KNeighborsClassifier
from sklearn.svm import SVC
from sklearn.naive_bayes import GaussianNB
from sklearn.preprocessing import StandardScaler
from sklearn.pipeline import make_pipeline
from sklearn.ensemble import StackingClassifier
from sklearn.ensemble import ExtraTreesClassifier, AdaBoostClassifier
import warnings
from sklearn.exceptions import DataConversionWarning
import numpy as np
from imblearn.over_sampling import RandomOverSampler
class myclf_MCS:
    # read file
    def read_file(input_file1):
        global df ,df_col_0
        file_name = os.path.splitext(input_file1)[0][:8]
        if os.path.splitext(input_file1)[1] == '.txt':
            df = pd.read_table(input_file1)
        elif os.path.splitext(input_file1)[1] == '.csv':
            df = pd.read_csv(input_file1)
        elif os.path.splitext(input_file1)[1] == '.xlsx' or os.path.splitext(input_file1)[1] == '.xls':
            df = pd.read_excel(input_file1)
        df_col_0 = df.columns[0]
        if not os.path.exists('./clf_MCS'):
            os.mkdir('./clf_MCS')
    def read_file(input_file2):
        global df ,df_col_0
        file_name = os.path.splitext(input_file2)[0][:8]
        if os.path.splitext(input_file2)[1] == '.txt':
            df = pd.read_table(input_file2)
        elif os.path.splitext(input_file2)[1] == '.csv':
            df = pd.read_csv(input_file2)
        elif os.path.splitext(input_file2)[1] == '.xlsx' or os.path.splitext(input_file2)[1] == '.xls':
            df = pd.read_excel(input_file2)
        df_col_0 = df.columns[0]
        if not os.path.exists('./clf_MCS'):
            os.mkdir('./clf_MCS')

    def mode_choose():
        global df, df_col_0
        if mode_choose == 'load_mode':
            read_file2(input_file2)
        else:
            read_file1(input_file1)

    
    def input_shuffle():
        global df
        df.dropna(inplace=True)  
        df = shuffle(df)

    
    def split_file():
        global X ,y
        if not Y_lb_TF == 'T' and mode_choose == 'load_mode':
            X = df
            y = 'nan'
            print('')
        else:
            y = df.iloc[: ,0]
            X = df.iloc[: ,1:]
            
        
    def oversample_data():
        global X, y
        from imblearn.over_sampling import RandomOverSampler, SMOTE
        ros = RandomOverSampler()
        
        X_resampled, y_resampled = ros.fit_resample(X, y)
        X = X_resampled
        y = y_resampled        
    
    def split_file_data():
        global X_test ,y_test ,X_train ,y_train,df
        # define seed for reproducibility
        seed = 12
        # split data into training and testing datasets
        X_train, X_test, y_train, y_test = model_selection.train_test_split(X
                                                                            , y
                                                                            , test_size=rate
                                                                            , random_state=seed)
        X_test = np.ascontiguousarray(X_test)


    def MCS_fit():
        global stacking_classifier_clf
        
        model1 = RandomForestClassifier(n_estimators=100)
        model2 =  GradientBoostingClassifier()

        model4 =  DecisionTreeClassifier()
        model5 = KNeighborsClassifier()
        model6 =  SVC()
        model7 =  GaussianNB()
        model8 =  ExtraTreesClassifier()
        model9 =  AdaBoostClassifier()

        
        meta_model = RandomForestClassifier()
        
        estimators = [
            ('rf', model1),
            ('gb', model2),

            ('dt', model4),
            ('knn', model5),
            ('svc', model6),
            ('nb', model7),
            ('et', model8),
            ('ada', model9)
        ]

        stacking_classifier_clf = StackingClassifier(
            estimators=estimators,
            final_estimator=meta_model
        )
        warnings.filterwarnings('ignore')

        
        stacking_classifier_clf.fit(X_train, y_train)




    def MCS_fit_diy(MCS_meta_model
                   ):
        global stacking_classifier_clf
        model1 = RandomForestClassifier(n_estimators=100)
        model2 =  GradientBoostingClassifier()

        model4 =  DecisionTreeClassifier()
        model5 = KNeighborsClassifier()
        model6 =  SVC()
        model7 =  GaussianNB()
        model8 =  ExtraTreesClassifier()
        model9 =  AdaBoostClassifier()

        
        if MCS_meta_model == 'random_forest':
            meta_model = RandomForestClassifier()
        elif MCS_meta_model == 'gradient_boosting':
            meta_model = GradientBoostingClassifier()
        elif MCS_meta_model == 'logistic_regression':
            meta_model = LogisticRegression()
        elif MCS_meta_model == 'ada_boost':
            meta_model = AdaBoostClassifier()
        else:
            raise ValueError("Invalid value for MCS_meta_model")

        
        estimators = [
            ('rf', model1),
            ('gb', model2),

            ('dt', model4),
            ('knn', model5),
            ('svc', model6),
            ('nb', model7),
            ('et', model8),
            ('ada', model9)
        ]

        stacking_classifier_clf = StackingClassifier(
            estimators=estimators,
            final_estimator=meta_model
        )
        warnings.filterwarnings('ignore')
        stacking_classifier_clf.fit(X_train, y_train)

        print('MCS-diy:', MCS_meta_model)

    def MCS_fit_cv(cv_times=5, cv_scoring=None):

        global stacking_classifier_clf, best_para
        model1 = RandomForestClassifier(n_estimators=100)
        model2 =  GradientBoostingClassifier()

        model4 =  DecisionTreeClassifier()
        model5 = KNeighborsClassifier()
        model6 =  SVC()
        model7 =  GaussianNB()
        model8 =  ExtraTreesClassifier()
        model9 =  AdaBoostClassifier()
        
        meta_model = RandomForestClassifier()
       
        estimators = [
            ('rf', model1),
            ('gb', model2),

            ('dt', model4),
            ('knn', model5),
            ('svc', model6),
            ('nb', model7),
            ('et', model8),
            ('ada', model9)
        ]

        meta_model = RandomForestClassifier(random_state=42)
        warnings.filterwarnings('ignore')

        
        parameters = {    'final_estimator__n_estimators': [25,50, 100, 200],
    'final_estimator__max_depth': [3,5,6,8,10],
   'final_estimator__criterion':['gini' , 'entropy']}
        clf = StackingClassifier(estimators=estimators,
            final_estimator=meta_model)
        stacking_classifier_clf = GridSearchCV(clf, parameters
                                , n_jobs=-1
                                , cv=cv_times
                                , verbose=1
                                , scoring=cv_scoring)
        stacking_classifier_clf.fit(X_train, y_train)
        best_para = list(stacking_classifier_clf.best_params_.values())
        print('', stacking_classifier_clf.best_params_)

   
    def save_model():
        joblib.dump(stacking_classifier_clf, './clf_MCS/MCS.pkl')

    def model_input():
        global stacking_classifier_clf
        model_name = os.path.splitext(input_model)[0][:8]
        stacking_classifier_clf = input_model

    def load_model(file_name='MCS'):
        global fpr_MCS, tpr_MCS, stacking_classifier_clf, predictions
        if mode_choose == 'load_mode':
            stacking_classifier_clf = joblib.load(stacking_classifier_clf)
        score_MCS = stacking_classifier_clf.predict_proba(X_test)[:,1]
        #fpr_MCS,tpr_MCS,thres_MCS = roc_curve(y_test,score_MCS,)
        predictions = stacking_classifier_clf.predict(X_test)
        print("MCS")
        confusion = confusion_matrix(y_test, predictions)
        Accuracy = (confusion[0][0]+confusion[1][1])/(confusion[0][0]+confusion[0][1]+confusion[1][0]+confusion[1][1])
        Precision = confusion[0][0]/(confusion[0][0]+confusion[0][1])
        Sensitivity = confusion[0][0]/(confusion[0][0]+confusion[1][0])
        Specifcity = confusion[1][1]/(confusion[1][1]+confusion[0][1])
        F1 = 2*Precision*Sensitivity/(Precision+Sensitivity)
        print("Accuracy:", Accuracy, "Precision:", Precision, "F1:",F1,"Sensitivity:",Sensitivity,"Specifcity:",Specifcity)
        print(classification_report(y_test, predictions))
        report = classification_report(y_test, predictions,output_dict=True)
        report = pd.DataFrame(report).transpose()
        report.rename(columns={'precision':'','recall':'','f1-score':'','support':''},inplace=True)
        report.rename(index={'accuracy':'','macro avg':'','micro avg':'','weighted avg':''},inplace=True)
        report.loc['',['','']] = [Sensitivity,Specifcity]
        #if mode_choose == 'train' and cv_TF == 'T':
        #    report.loc['',['priors']] = [best_para[0]]
        report.to_csv('./clf_MCS/{}_Report.txt'.format(file_name),index = 1,index_label='',sep = '\t')

    def load_model_misslb(model_name='MCS'):
            global stacking_classifier_clf
            MCS_clf = joblib.load(stacking_classifier_clf)
            predic = MCS_clf.predict(X_test)
            predic = pd.DataFrame(predic)
            pd_data = pd.concat([predic, X], axis=1)
            pd_data.rename(columns={0: ''}, inplace=True)
            pd_data.to_csv('./clf_MCS/{}_PredictData.txt'.format(model_name), index=0, sep='\t')

    def pd_concat(model_name='MCS'):
            predic = pd.DataFrame(predictions)
            pd_col_0 = predic.columns[0]
            pd_data = pd.concat([predic, X_test], axis=1)
            # pd_data.rename(columns={pd_col_0:''},inplace=True)
            pd_data = pd.concat([y_test, pd_data], axis=1)
            pd_data.rename(columns={pd_col_0: '', df_col_0: ''}, inplace=True)
            pd_data.to_csv('./clf_MCS/{}_PredictData.txt'.format(model_name), index=0, sep='\t')

    #def plt_output(model_name='MCS'):
            #fig, ax = plt.subplots(figsize=(10, 8))
            #ax.plot(fpr_MCS, tpr_MCS, linewidth=2, color="black",
            #        label='MCS (AUC={})'.format(str(round(auc(fpr_MCS, tpr_MCS), 3))))
            #ax.plot([0, 1], [0, 1], linestyle='--', color='grey')
            
            #plt.legend(fontsize=12)
            #plt.title("Validation Cohort ROC")
            #plt.xlabel("False Positive Rate")
            #plt.ylabel("True Positive Rate")
            # plt.show()
            #f = plt.gcf()  
            #if mode_choose == 'train':
            #    f.savefig('./clf_MCS/MCS_ROC.svg', bbox_inches='tight')
            #else:
            #    f.savefig('./clf_MCS/{}_ROC.svg'.format(model_name), bbox_inches='tight')

    def split_model_pddata():
            global X_test, y_test
            X_test = X
            y_test = y
            #print(y_test)


    def zipDir(source_dir, output_filename):
            
            if os.path.exists(source_dir):
                zipf = zipfile.ZipFile(file=output_filename, mode='w', compression=zipfile.ZIP_LZMA)
                # zipf = zipfile.ZipFile(file=output_filename, mode='w', compression=zipfile.ZIP_DEFLATED)
                pre_len = len(os.path.dirname(source_dir))
                for parent, dirnames, filenames in os.walk(source_dir):
                    for filename in filenames:
                        pathfile = os.path.join(parent, filename)
                        arcname = pathfile[pre_len:].strip(os.path.sep)  
                        zipf.write(pathfile, arcname)
                zipf.close()
                print('', source_dir, '', output_filename)
                return output_filename
            else:
                print('file not found ', source_dir)
            return

def train_mode(input_file1):
    myclf_MCS.read_file(input_file1)
    if shuffle_TF == 'T':
            myclf_MCS.input_shuffle()
    myclf_MCS.split_file()
    myclf_MCS.split_model_pddata()
    myclf_MCS.split_file_data()
    if cv_TF == 'T':
        myclf_MCS.MCS_fit_cv()
    else:
        if MCS_diy_TF == 'T':
            myclf_MCS.MCS_fit_diy(MCS_meta_model)
        else:
            myclf_MCS.MCS_fit()
    myclf_MCS.save_model()
    myclf_MCS.load_model()
    #myclf_MCS.plt_output()
    myclf_MCS.zipDir(source_dir, output_filename)

def load_mode(input_file2):
    myclf_MCS.read_file(input_file2)
    myclf_MCS.split_file()
    myclf_MCS.split_model_pddata()
    myclf_MCS.model_input()
    if Y_lb_TF == 'T':
        myclf_MCS.load_model()
        myclf_MCS.pd_concat()
        #myclf_MCS.plt_output()
        myclf_MCS.zipDir(source_dir, output_filename)
    else:
        myclf_MCS.load_model_misslb()
        myclf_MCS.zipDir(source_dir, output_filename)
warnings.filterwarnings("ignore", category=DataConversionWarning)        

    # argv

source_dir = './clf_MCS'
output_filename = './clf_MCS.zip'

argv = sys.argv
mode_choose = argv[1]  # train nan train.csv
Y_lb_TF = argv[2]
input_model = argv[3]  # load_mode MCS.pkl pd.csv
input_file1 = argv[4]
input_file2 = argv[5]
rate = float(argv[6])

shuffle_TF = argv[7]

cv_TF = argv[8]
cv_times = argv[9]
cv_scoring = argv[10]

MCS_diy_TF = argv[11]
if MCS_diy_TF == 'T':
    MCS_meta_model = argv[12]


    if cv_scoring == 'None':
        cv_scoring = None
        # python clf_SVC.py load_mode F GBMCS.pkl train.csv 0.2 T 1 2 3 10 11 12 13 14 15

        # python clf_GBNB.py train T NAN train.csv 0.2 T T 5 None 0

        # GBNB_loss,GBNB_learning_rate,GBNB_subsample,GBNB_n_estimators,GBNB_criterion,GBNB_max_depth,GBNB_min_samples_leaf,GBNB_min_samples_split,GBNB_max_features

if __name__ == '__main__':
    if mode_choose == 'train':
        train_mode(input_file1)
    elif mode_choose == 'load_mode':
        load_mode(input_file2)
        print('')
