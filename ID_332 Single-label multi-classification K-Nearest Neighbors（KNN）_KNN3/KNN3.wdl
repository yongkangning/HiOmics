...
task task_KNN3 {
  ...
  command <<<
    ...
    python ~{appBinDir}/clf_KNN3.py ~{mode_choose} ~{Y_lb_TF} ~{input_model} ~{input_file1} ~{input_file2} ~{rate} ~{shuffle_TF} ~{cv_TF} ~{cv_times} ~{cv_scoring} ~{KNN_diy_TF} ~{KNN_n_neighbors} ~{KNN_algorithm} ~{KNN_metric} ~{KNN_leaf_size}
    >>>
  output {
