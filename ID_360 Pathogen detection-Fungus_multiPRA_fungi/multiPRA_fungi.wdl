...
task task_multiPRA_fungi {
    ...
    command <<<
        ...
        -p ${cpu_cores} \
        --db Fungi -O $HOME/result

        #cp $HOME/result/report.docx  ./
        find $HOME/result/bak/mg/stat/ -name "positive.strain.*.txt" -exec mv {} ./pathogene.fungi.xls \;
    >>>

    output {
        File? result = "pathogene.fungi.xls"
    }
