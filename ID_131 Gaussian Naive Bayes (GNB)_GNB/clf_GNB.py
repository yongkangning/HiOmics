...
class myclf_NB:
    ...
    def NB_fit_diy(NB_var_smoothing=0.000000001
                   ,NB_priors=[0.3]
                   ):
        global NB_clf
        NB_clf = naive_bayes.GaussianNB(var_smoothing=NB_var_smoothing
                                    ,priors=NB_priors
                                    )
        NB_clf.fit(X_train, y_train)
        print('NB-diy',NB_var_smoothing
                      ,NB_priors
                      )
    argv = sys.argv
    cv_times = int(argv[9])    
    ...
    def save_model():
        joblib.dump(NB_clf, './clf_NB/GNB.pkl')
...
def train_mode(input_file1):
    ...
    if cv_TF == 'T':
        ...
        if NB_diy_TF == 'T':
            myclf_NB.NB_fit_diy(NB_var_smoothing
                                ,NB_priors)
        else:
            myclf_NB.NB_fit()
    myclf_NB.save_model()
...
NB_diy_TF = argv[11]
if NB_diy_TF == 'T':
    NB_var_smoothing = float(argv[12])
    NB_priors = myclf_NB.split_float_name(argv[13])
