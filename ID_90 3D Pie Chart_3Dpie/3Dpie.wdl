...
task task_3Dpie {
  ...
  command <<<
    ...
    # genarate the result
    Rscript ~{appBinDir}/3Dpie_docker_v.1.0.R ~{RLibPath} inputFile2.txt ~{is_legend} ~{is_explode} ~{is_height} ~{is_radius} ~{label_ces} ~{outputFileName}
...
workflow henbio_wf {
  ...
  output {
    File outFile = task_3Dpie.outFile
  }
}