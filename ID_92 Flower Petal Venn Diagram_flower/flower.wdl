...
task task_flower {
  ...
  command <<<
    ...
    # genarate the result
    Rscript ~{appBinDir}/flower_docker_v.1.0.R ~{RLibPath} inputFile2.txt ~{group_cex} ~{core_name} ~{is_start} ~{r_size} ~{cir_color} ~{outputFileName}
...
workflow henbio_wf {
  ...
  output {
    File outFile = task_flower.outFile
  }
}