...
task task_xiangxingtu_boxplot {
  ...
  command <<<
    ...
    # genarate the result
    Rscript ~{appBinDir}/xiangxingtu_boxplot_docker_v.1.3.R ~{RLibPath} inputFile2.txt ~{is_alpha} ~{Palette} ~{x_title} ~{y_title} ~{plot_title} ~{is_method} ~{p_sig} ~{is_legend} ~{outputFileName}
...
workflow henbio_wf {
  call task_xiangxingtu_boxplot

  output {
    File outFile = task_xiangxingtu_boxplot.outFile
  }
}