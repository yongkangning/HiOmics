...
if (length(args)!=argsCount) {
  stop("xiangxingtu_boxplot_docker_v.1.2.R libPath inputFile is_alpha Palette x_title y_title plot_title is_method p_sig is_legend outputFileName")
}
.libPaths(args[1])
...
p1 = ggboxplot(data, x=colnames(data)[1], y=colnames(data)[2], color=colnames(data)[1],
             #fill = "stage",
		     #legend.title=x,
			  alpha=is_alpha,
              palette = Palette,
...
}else if(is_method %in% is_method5){if(p_sig %in% pval){p5_1=p1+stat_compare_means(comparisons = my_comparisons,method = "kruskal.test")
                                                         ggsave(plot=p5_1,file=paste(outputFileName,"svg",sep="."),device='svg',units="cm",width=13,height=13,dpi=600)
                              }else if(p_sig %in% sig){p5_2=p1+stat_compare_means(comparisons = my_comparisons,method ="kruskal.test",label = "p.signif")
