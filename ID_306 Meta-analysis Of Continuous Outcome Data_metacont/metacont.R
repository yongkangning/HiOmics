...
if (length(args)!=argsCount) {
  stop("metacont.R libPath inputFile is_sm")
}
.libPaths(args[1])
...
library(meta)
meta2=metacont(data[,2],data[,3],data[,4],data[,5],data[,6],data[,7],data=data, studlab=data[,1],sm=is_sm)
svg(filename = "Forest.svg",height =length(data[,1])*0.7 ,width = 12)
forest(meta2,digits.se =2,
...
metabias(meta2,plotit=TRUE,k.min = length(data[,1]))
dev.off()

svg(filename = "Sensitivity.svg",height =length(data[,1])*0.7 ,width = 12)
forest(metainf(meta2),
       col.diamond.fixed="lightslategray",
       col.diamond.lines.fixed="lightslategray",
       col.diamond.random="maroon",
       col.diamond.lines.random="maroon",
