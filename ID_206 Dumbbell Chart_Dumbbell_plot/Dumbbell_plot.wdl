...
task task_Dumbbell_plot {
  ...
  command <<<
    ...
    # genarate the result
    Rscript ~{appBinDir}/Dumbbell_plot_docker.R ~{RLibPath} inputFile2.txt ~{x_title} ~{y_title} ~{outputFileName}
...
workflow henbio_wf {
  call task_Dumbbell_plot

  output {
    File outFile = task_Dumbbell_plot.outFile
  }
}